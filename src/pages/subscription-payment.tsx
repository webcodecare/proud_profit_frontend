...
export default function SubscriptionPayment() {
  ...
  const handleStripePayment = async (tier: PricingTier) => {
    // Redirect to Stripe checkout page with plan details
    const amount = tier.price * 100; // Convert to cents for Stripe
    window.location.href = `/checkout?plan=${tier.id}&amount=${amount}&billing=monthly`;
  };

  const handlePayPalPayment = async (tier: PricingTier) => {
    // Redirect to PayPal checkout page with plan details
    window.location.href = `/checkout-paypal?plan=${tier.id}&amount=${tier.price}&billing=monthly`;
  };
