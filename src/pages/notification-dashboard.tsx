...
export default function AlertNotificationDashboard() {
          ...
          <header className="bg-card border-b border-border p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold">Alert Notification Dashboard</h1>
              ...
              <CardContent>
                {logsLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm font-medium text-red-600 mb-1">Failed to load notification logs</p>
                    <p className="text-xs text-muted-foreground mb-4">{(logsError as Error).message || 'An unexpected error occurred'}</p>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        ...
                        toast({
                          title: "Retrying",
                          description: "Attempting to reload failure logs...",
                        });
                      }}
                    >
                      <RefreshCw className="h-4 w-4 mr-2" />
                  ...
                  <div className="space-y-3">
                    {(() => {
                      const failedLogs = (logs as NotificationLog[])
                      ...
                      if (failedLogs.length === 0) {
                        return (
                          <div className="text-center py-8">
                            ...
                            <p className="text-sm text-muted-foreground">No recent failures found</p>
                            <p className="text-xs text-muted-foreground mt-1">All notifications are being delivered successfully</p>
                          </div>
                        );
                      }
                      
                      return failedLogs.map((failure) => (
                        <div key={failure.id} className="border rounded-lg p-4 bg-red-50 dark:bg-red-950/20">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-3 mb-2">
                                <Badge variant="destructive">{failure.type.toUpperCase()}</Badge>
                                <span className="font-semibold">{failure.recipient}</span>
                                <span className="text-xs text-muted-foreground">User: {failure.userId}</span>
                              </div>
                              <div className="grid grid-cols-2 gap-2 text-sm">
                                <div>
                                  <span className="text-muted-foreground">Failure Reason:</span>
                                  <p className="font-medium text-red-600">{failure.failureReason || 'Unknown error'}</p>
                                </div>
                                <div>
                                  <span className="text-muted-foreground">Alert:</span>
                                  <p className="font-medium">{failure.ticker || 'N/A'} - {failure.alertType.toUpperCase()}</p>
                                </div>
                                <div>
                                  <span className="text-muted-foreground">Time:</span>
                                  <p className="font-medium">{failure.sentAt || 'N/A'}</p>
                                </div>
                                <div>
                                  <span className="text-muted-foreground">Retry Count:</span>
                                  <p className="font-medium">{failure.attempts}/3</p>
                                </div>
                              </div>
                            </div>
                            <div className="flex flex-col space-y-2">
                              <Button 
                                size="sm" 
                                variant="outline" 
                                onClick={() => retryNotificationMutation.mutate(failure.id)}
                                disabled={retryNotificationMutation.isPending}
                              >
                                <RefreshCw className={`h-3 w-3 mr-1 ${retryNotificationMutation.isPending ? 'animate-spin' : ''}`} />
                                Retry
                              </Button>
