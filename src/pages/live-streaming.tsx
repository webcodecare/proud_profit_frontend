...
export default function LiveStreamingPage() {
  ...
  const { data: streamingStatus, isLoading, refetch } = useQuery<StreamingStatus>({
    ...
    refetchInterval: isStreaming ? 30000 : false, // Only poll when streaming
        ...
        try {
          ...
          if (message.type === 'signal') {
            ...
            toast({
              ...
              description: `${signal.symbol} at $${Number(signal.price).toLocaleString()}`,
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Enabled symbols</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-xl font-bold text-purple-600">
                    {streamingStatus?.server?.uptime ? formatUptime(streamingStatus.server.uptime) : "N/A"}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {streamingStatus?.server?.memory?.heapUsed ? formatMemory(streamingStatus.server.memory.heapUsed) : "N/A"} used
                  </p>
                </CardContent>
              </Card>
            </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      )) || <span className="text-muted-foreground text-sm">No symbols enabled</span>}
                    </div>
                  </div>
                    ...
                    <div className="lg:hidden p-4 space-y-4">
                      {liveSignals.map((signal) => (
                        <Card key={signal.id} className="p-3">
                          ...
                          {signal.note && (
                            <p className="text-xs text-muted-foreground mt-2">{signal.note}</p>
                          )}
                        </Card>
                      ))}
                    </div>
                      ...
                      <table className="w-full">
                        <thead className="bg-muted/50">
                          <tr>
                            ...
                            <th className="text-left p-4 font-medium text-muted-foreground">Signal</th>
