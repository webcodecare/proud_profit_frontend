...
export default function HistoricalOHLCPage() {
  ...
  const { data: ohlcResponse, isLoading, refetch, error } = useQuery<OHLCResponse>({
    queryKey: ['/api/market/ohlc', selectedSymbol, selectedInterval, selectedLimit, currentPage],
    queryFn: async () => {
      console.log(`📊 Fetching OHLC data for ${selectedSymbol} ${selectedInterval} limit:${selectedLimit} page:${currentPage}`);
      
      try {
        // Use the correct backend API endpoint
        const data = await apiRequest(`/api/market/ohlc/${selectedSymbol}?interval=${selectedInterval}&limit=${selectedLimit}&page=${currentPage}`);
        console.log('✅ Backend API response:', data);
        
        // Transform timestamp to time format if needed
        if (data.data && Array.isArray(data.data)) {
          data.data = data.data.map((candle: any) => ({
            ...candle,
            time: candle.time || new Date(candle.timestamp).toISOString()
          }));
        }
        
        return data;
      } catch (backendError) {
        // Fallback to Binance API with CORS proxy
        console.log('⚠️ Backend failed, using Binance API fallback');
        console.error('Backend error:', backendError);
        
        try {
          const binanceInterval = selectedInterval;
          const binanceUrl = `https://api.binance.com/api/v3/klines?symbol=${selectedSymbol}&interval=${binanceInterval}&limit=${selectedLimit}`;
          ...
          console.log('✅ Binance data received:', binanceData.length, 'candles');
          
          // Transform Binance data to our format
          const transformedData = binanceData.map((candle: any) => ({
  ...
  useEffect(() => {
    if (ohlcResponse && ohlcResponse.totalPages) {
      setTotalPages(ohlcResponse.totalPages);
    } else if (ohlcResponse && ohlcResponse.data && ohlcResponse.data.length > 0) {
      // Backend doesn't return pagination metadata, so estimate based on data
      // If we got a full page of data, assume there might be more
      const limit = parseInt(selectedLimit);
      if (ohlcResponse.data.length >= limit) {
  ...
  }, [ohlcResponse, currentPage, selectedLimit]);
  ...
  const calculateStats = () => {
    ...
    const high24h = Math.max(...ohlcData.slice(0, 24).map((d: OHLCData) => d.high));
    const low24h = Math.min(...ohlcData.slice(0, 24).map((d: OHLCData) => d.low));
    const volume24h = ohlcData.slice(0, 24).reduce((sum: number, d: OHLCData) => sum + d.volume, 0);
          ...
          <div className="p-4 lg:p-6 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="p-0">
                ...
                {/* Pagination Controls */}
                {ohlcData.length > 0 && (currentPage > 1 || totalPages > currentPage || (ohlcResponse?.hasNext)) && (
                  <div className="px-4 py-4 border-t">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="text-sm text-muted-foreground">
                        Page {ohlcResponse?.currentPage || currentPage} 
                        {ohlcResponse?.totalPages && <span> of {ohlcResponse.totalPages}</span>}
                        {ohlcResponse?.totalCount && (
                          <span> • {ohlcResponse.totalCount} total records</span>
                        )}
                        {!ohlcResponse?.totalPages && ohlcData.length >= parseInt(selectedLimit) && (
                          <span> • More pages may be available</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          disabled={currentPage <= 1 || (ohlcResponse?.hasPrevious === false)}
                        >
                          Previous
                        </Button>
                        ...
                        <Button
                          ...
                          disabled={
                            ...
                            (ohlcResponse?.hasNext === false) ||
                            (ohlcResponse?.totalPages && currentPage >= ohlcResponse.totalPages) ||
                            // Fallback: if we got less data than requested, we're at the end
                            (!ohlcResponse?.hasNext && !ohlcResponse?.totalPages && ohlcData.length < parseInt(selectedLimit))
                          }
                        >
                          Next
                        </Button>
