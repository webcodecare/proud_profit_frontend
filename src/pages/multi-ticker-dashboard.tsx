...
import SimpleTradingViewChart from "@/components/charts/SimpleTradingViewChart";
...
const AdvancedForecastChart = lazy(
  () => import("@/components/charts/AdvancedForecastChart")
);
...
export default function MultiTickerDashboard() {
  ...
  // Extract symbols from watchlist for backward compatibility
  const selectedTickers = watchlistData?.watchlist?.map(
    (item: WatchlistItem) => item.symbol
  ) || ["BTCUSDT", "ETHUSDT", "SOLUSDT"];

  const [selectedChart, setSelectedChart] = useState<string>(
    selectedTickers[0] || "BTCUSDT"
  );

  // Track which cryptocurrency is currently being operated on
  ...
  const { data: pricesData, isLoading: pricesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Backend returns {data: [...]} format, convert to {prices: {symbol: data}} format
  ...
  // Fetch public signals - filter by selected tickers
  ...
  const tickers: TickerData[] = React.useMemo(() => {
  ...
  // Update signals - filter to only show signals for selected tickers
  useEffect(() => {
    if (signalsData?.signals) {
      const filteredSignals = signalsData.signals.filter((signal: any) =>
        selectedTickers.some((ticker: string) => {
          // Normalize both ticker and signal for comparison - remove common quote currencies
          ...
          // Match if normalized symbols are equal OR if exact match
          return (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-sm sm:text-base text-muted-foreground mt-1">
                  Tracking {selectedTickers.length} cryptocurrencies with live
                  data
                </p>
              </div>
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                >
                  <Activity className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4 animate-pulse" />
                ...
                >
                  ...
                  <span className="sm:hidden">{recentSignals.length}</span>
                </Button>
              </div>
            </div>
          </header>
              ...
              <Tabs defaultValue="overview" className="w-full">
                        ...
                        <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                          <div className="flex items-center justify-between">
                            <div className="text-sm text-muted-foreground">
                              {watchlistData.watchlist.length} cryptocurrencies
                              in your watchlist
                            </div>
                            {operatingSymbol && (
                ...
                <TabsContent value="charts" className="space-y-6">
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <CardTitle>Interactive Price Charts</CardTitle>
                        <div className="flex items-center gap-2">
                          <Label className="text-sm">Select Ticker:</Label>
                          <Select
                            value={selectedChart}
                    ...
                    <CardContent>
                      {selectedTickers.length === 0 ? (
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                          Select cryptocurrencies from your watchlist to view
                          charts
                        </div>
                      ) : (
                  ...
                  {tickers.length > 1 && (
                    <Card>
                      <CardHeader>
                        <CardTitle>Quick View - Selected Tickers</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {tickers.slice(0, 6).map((ticker) => (
                            <Button
                              key={ticker.symbol}
                              variant={
                                selectedChart === ticker.symbol
                                  ? "default"
                                  : "outline"
                              }
                              onClick={() => setSelectedChart(ticker.symbol)}
                    ...
                    <Card>
                      <CardContent className="p-6">
                        <div className="space-y-2">
                          ...
                          <p className="text-3xl font-bold">{tickers.length}</p>
                          <p className="text-xs text-muted-foreground">
                    ...
                    <Card>
                      <CardContent className="p-6">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {tickers.length > 0 ? "Last 24 hours" : "No data"}
                          </p>
                        </div>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent className="p-6">
                        <div className="space-y-2">
                          ...
                          <p className="text-3xl font-bold">
                            ...
                            ) : tickers.filter((t) => t.changePercent24h >= 0)
                                .length >
                              tickers.length / 2 ? (
