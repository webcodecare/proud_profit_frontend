...
export default function Settings() {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (updates: any) => {
      ...
      // If email changed, we need the current password for security
      if (emailChanged && !updates.currentPassword) {
    ...
    onSuccess: ({ data, emailChanged }) => {
      toast({
        ...
        description: emailChanged 
          ? "Your profile and email have been updated. Please use your new email to sign in." 
      ...
      // If email changed, suggest user to re-login
      if (emailChanged) {
        setTimeout(() => {
          toast({
            title: "Email Changed",
            description: "Please sign in again with your new email address.",
            variant: "default",
          });
          logout();
        }, 2000);
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (updates: any) => {
      ...
      const dbUpdates = {
        ...
        notification_telegram: updates.notificationTelegram,
        email_address: updates.emailAddress,
        phone_number: updates.phoneNumber,
        telegram_chat_id: updates.telegramChatId,
        price_alerts: updates.priceAlerts,
        volume_alerts: updates.volumeAlerts,
        news_alerts: updates.newsAlerts,
        technical_alerts: updates.technicalAlerts,
        whale_alerts: updates.whaleAlerts,
        push_enabled: updates.pushEnabled,
        push_subscription: updates.pushSubscription,
        updated_at: new Date().toISOString(),
      };
      ...
      if (error) {
        ...
        if (error.message?.includes('row-level security') || error.code === '42501' || error.code === 'PGRST301') {
          throw new Error('Permission denied: Please run the FIX_USERS_RLS.sql file in your Supabase SQL Editor to enable settings updates.');
        }
        throw error;
      }
      return data;
