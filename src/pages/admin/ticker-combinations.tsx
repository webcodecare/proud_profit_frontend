...
export default function AdminTickerCombinations() {
  ...
  const { data: combinations, isLoading: isLoadingCombinations } = useQuery({
    ...
    queryFn: async () => {
      ...
      return combosRes.data?.map((combo: any) => ({
        id: combo.id,
        tickerSymbol: tickerMap.get(combo.ticker_id) || '',
  ...
  const createCombinationMutation = useMutation({
    mutationFn: async (combinationData: typeof newCombination) => {
      // Insert with just timeframe and price - ticker_id will use default or we'll use the API
      const response = await fetch(buildApiUrl('/api/admin/ticker-timeframes'), {
        method: 'POST',
        headers: { 
        ...
        body: JSON.stringify({
          ticker_symbol: combinationData.tickerSymbol,
          timeframe: combinationData.timeframe,
          is_active: combinationData.isEnabled
        })
      });
  ...
  const updateCombinationMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<TickerTimeframeCombination> }) => {
      const { data, error } = await supabase
        ...
        .update({ is_active: updates.isEnabled })
        .eq('id', id)
        .select();
  ...
  const editCombinationMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<TickerTimeframeCombination> }) => {
      ...
      if (updates.tickerSymbol !== undefined) {
        const ticker = tickers?.find((t: Ticker) => t.symbol === updates.tickerSymbol);
        if (ticker) payload.ticker_id = ticker.id;
      }
      if (updates.timeframe !== undefined) {
        payload.timeframe = updates.timeframe;
      }
      if (updates.isEnabled !== undefined) {
        payload.is_active = updates.isEnabled;
      }
