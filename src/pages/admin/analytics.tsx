...
export default function AdminAnalytics() {
  ...
  const { data: analyticsData, isLoading, error } = useQuery({
    ...
    queryFn: () => apiRequest(`/api/admin/analytics?period=${selectedPeriod}${selectedMetric ? `&metric=${selectedMetric}` : ''}`, {
      method: 'GET',
  ...
  const chartData = (analyticsData?.chartData && analyticsData.chartData.length > 0) 
    ? analyticsData.chartData 
    : sampleChartData;
  
  const revenueChartData = (revenueData?.monthlyRevenue && revenueData.monthlyRevenue.length > 0)
    ? revenueData.monthlyRevenue
    : sampleRevenueData;
  
  const signalChartData = (signalPerformance?.accuracyData && signalPerformance.accuracyData.length > 0)
    ? signalPerformance.accuracyData
    : sampleSignalData;
  
  const userRegistrationData = (userMetrics?.registrationTrends && userMetrics.registrationTrends.length > 0)
          ...
          <Card className="w-full max-w-md">
            ...
            <CardContent className="space-y-4">
              <p className="text-muted-foreground">
                Unable to fetch analytics data. This could be due to API issues or connectivity problems.
              </p>
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {isLoading ? <Skeleton className="h-8 w-24" /> : overviewData.totalUsers.toLocaleString()}
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {isLoading ? <Skeleton className="h-8 w-24" /> : `$${overviewData.monthlyRevenue.toLocaleString()}`}
                  </div>
                  <div className="flex items-center text-xs text-muted-foreground">
                    <TrendingUp className="h-3 w-3 mr-1 text-green-500" />
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {isLoading ? <Skeleton className="h-8 w-24" /> : (analyticsData?.overview?.activeSubscriptions || 0).toLocaleString()}
                  </div>
                  <div className="flex items-center text-xs text-muted-foreground">
                    <TrendingUp className="h-3 w-3 mr-1 text-green-500" />
                    +{analyticsData?.overview?.subscriptionGrowth || 0}% from last period
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {isLoading ? <Skeleton className="h-8 w-20" /> : `${overviewData.signalAccuracy}%`}
                  </div>
                  <div className="flex items-center text-xs text-muted-foreground">
                    <TrendingUp className="h-3 w-3 mr-1 text-green-500" />
                ...
                {isLoading ? (
                  ...
                  <div className="h-[320px] flex items-center justify-center border-2 border-dashed rounded-lg">
                    <div className="text-center p-6">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">Try selecting a different time period</p>
                    </div>
                  </div>
                )}
              </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-4 bg-muted/20 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{overviewData.activeUsers.toLocaleString()}</div>
                  <div className="text-sm text-muted-foreground">Active Users (30d)</div>
                </div>
                <div className="text-center p-4 bg-muted/20 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{analyticsData?.overview?.retentionRate || 0}%</div>
                  <div className="text-sm text-muted-foreground">User Retention Rate</div>
                </div>
                <div className="text-center p-4 bg-muted/20 rounded-lg">
                  <div className="text-2xl font-bold text-purple-600">{analyticsData?.overview?.avgSessionDuration || 0}</div>
                ...
                {userMetricsLoading ? (
                  ...
                  <div className="h-[320px] flex items-center justify-center border-2 border-dashed rounded-lg">
                    <div className="text-center p-6">
                      ...
                      <p className="text-sm font-medium text-muted-foreground">No user registration data</p>
                      <p className="text-xs text-muted-foreground mt-1">Data will appear here once users register</p>
                    </div>
                  </div>
                )}
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant="outline">{userMetrics?.dailyActiveUsers || 0}</Badge>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Weekly Active Users</span>
                    <Badge variant="outline">{userMetrics?.weeklyActiveUsers || 0}</Badge>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Monthly Active Users</span>
                    <Badge variant="outline">{userMetrics?.monthlyActiveUsers || 0}</Badge>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Avg. Session Time</span>
                    <Badge variant="outline">{userMetrics?.avgSessionTime || '0m'}</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  {revenueLoading ? (
                    ...
                    <div className="h-full flex items-center justify-center border-2 border-dashed rounded-lg">
                      <div className="text-center p-6">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Revenue metrics will be displayed here</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">${revenueData?.sources?.subscriptions?.toLocaleString() || 0}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Trading Fees</span>
                    <span className="font-medium">${revenueData?.sources?.tradingFees?.toLocaleString() || 0}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Premium Features</span>
                    <span className="font-medium">${revenueData?.sources?.premiumFeatures?.toLocaleString() || 0}</span>
                  </div>
                  <div className="border-t pt-4">
                    <div className="flex justify-between items-center font-bold">
                      <span>Total</span>
                      <span>${overviewData.monthlyRevenue.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
                  ...
                  {signalLoading ? (
                    ...
                    <div className="h-full flex items-center justify-center border-2 border-dashed rounded-lg">
                      <div className="text-center p-6">
                        ...
                        <p className="text-sm font-medium text-muted-foreground">No signal performance data</p>
                        <p className="text-xs text-muted-foreground mt-1">Signal metrics will appear once available</p>
                      </div>
                    </div>
                  )}
                </div>
