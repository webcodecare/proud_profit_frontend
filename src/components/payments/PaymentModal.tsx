...
export default function PaymentModal({
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl('/api/create-subscription'), {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          successUrl: `${origin}/dashboard?payment=success&session_token=${authToken}`,
          cancelUrl: `${origin}/pricing?payment=cancelled&session_token=${authToken}`
        })
      });
      ...
      if (response.ok && data.checkoutUrl) {
        console.log('🔐 Redirecting to Stripe with auth token preserved');
