...
const loadTradingViewScript = (): Promise<void> => {
  ...
  tradingViewScriptPromise = new Promise(async (resolve, reject) => {
    try {
      // Check if TradingView widget is already ready
      if (window.TradingView && window.TradingView.widget) {
        resolve();
        return;
      }
...
export default function TradingViewChart({
    ...
    const initWidget = async () => {
      try {
        ...
        if (!window.TradingView || !containerRef.current) {
          throw new Error('TradingView script not loaded or container not found');
        }
        ...
        // Create new widget
        const containerId = containerRef.current.id || `tradingview_${Date.now()}`;
  ...
  // Fetch OHLC data for chart using the working endpoint
      ...
      <div className="p-4 bg-slate-700 border-t border-slate-600">
        <div className="flex flex-wrap justify-center items-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-300">Price Line</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-0.5 bg-orange-400" style={{borderStyle: 'dashed'}}></div>
