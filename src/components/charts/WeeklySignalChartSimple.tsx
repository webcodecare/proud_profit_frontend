...
export default function WeeklySignalChartSimple({ 
  ...
  const loadChartData = async () => {
    try {
      ...
      // Process OHLC data - backend returns array directly
      if (ohlcResult.status === 'fulfilled' && Array.isArray(ohlcResult.value) && ohlcResult.value.length) {
        setOhlcData(ohlcResult.value);
        console.log('OHLC loaded:', ohlcResult.value.length, 'candles');
        console.log('Sample OHLC data:', ohlcResult.value[0]);
      } else {
        console.warn('OHLC fetch failed:', ohlcResult.status === 'rejected' ? ohlcResult.reason : 'No data');
      }

      // Process signals data - backend returns {signals: [...], total: number}
      if (signalsResult.status === 'fulfilled' && signalsResult.value?.signals?.length) {
        setSignals(signalsResult.value.signals);
        console.log('Signals loaded:', signalsResult.value.signals.length, 'signals');
      } else {
      ...
      console.log('Data loading summary:', { 
        ohlc: ohlcResult.status === 'fulfilled' ? (Array.isArray(ohlcResult.value) ? ohlcResult.value.length : 0) : 'failed',
        signals: signalsResult.status === 'fulfilled' ? (signalsResult.value?.signals?.length || 0) : 'failed',
