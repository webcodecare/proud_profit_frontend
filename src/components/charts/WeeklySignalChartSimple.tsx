...
export default function WeeklySignalChartSimple({ 
  ...
  const fetchJson = async (url: string, timeoutMs = 6000) => {
    ...
    try {
      ...
      // Handle AbortError specifically to prevent unhandled promise rejections
  ...
  const loadChartData = async () => {
    try {
      ...
      const [ohlcResult, signalsResult, priceResult] = await Promise.allSettled([
        fetchJson(`/api/public/ohlc?symbol=${ticker}&interval=1d&limit=104`),
        fetchJson(`/api/public/signals/alerts?ticker=${ticker}&timeframe=1h`),
        fetchJson(`/api/public/market/price/${ticker}`)
      ]);
  ...
  useEffect(() => {
    const initChart = async () => {
      try {
        ...
        // Type assertion needed due to mismatch between lightweight-charts definitions and runtime API
        ...
        console.log('Rendering fallback chart with data:', { ohlc: ohlcData.length, signals: signals.length, price: currentPrice });
        // Fallback to simple data display
        renderFallbackChart();
      }
    };

    console.log('useEffect triggered:', { ohlcLength: ohlcData.length, isLoading, hasData: ohlcData.length > 0 && !isLoading });
    
    if (ohlcData.length > 0) {
      console.log('Chart data ready, rendering fallback chart immediately');
  ...
  const renderFallbackChart = () => {
    try {
      console.log('Starting fallback chart render...');
  ...
  useEffect(() => {
    // Add unhandled promise rejection handler for debugging
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      console.warn('Unhandled promise rejection in WeeklySignalChart:', event.reason);
