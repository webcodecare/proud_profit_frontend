...
export default function BuySellSignalChart({ 
  ...
  const loadChartData = async () => {
    try {
      ...
      const [ohlcResult, priceResult] = await Promise.allSettled([
        fetch(buildApiUrl('/api/public/ohlc?symbol=BTCUSDT&interval=1w&limit=104')).then(r => r.json()),
      ...
      if (ohlcResult.status === 'fulfilled' && ohlcResult.value?.data?.length) {
        ...
        console.error('Failed to load OHLC data:', ohlcResult.reason);
      }
