...
export default function SimpleSignalsChart({ 
  ...
  const fetchSignals = async () => {
    try {
      ...
      const [signalsResult, priceResult] = await Promise.allSettled([
        fetch(buildApiUrl('/api/public/signals/alerts?ticker=BTCUSDT&timeframe=1W')).then(r => r.json()),
        fetch(buildApiUrl('/api/public/market/price/BTCUSDT')).then(r => r.json())
      ]);

      if (signalsResult.status === 'fulfilled' && signalsResult.value?.data?.length) {
        setSignals(signalsResult.value.data);
      ...
      if (priceResult.status === 'fulfilled' && priceResult.value?.price) {
        ...
        console.error('Failed to load price:', priceResult.reason);
      }
