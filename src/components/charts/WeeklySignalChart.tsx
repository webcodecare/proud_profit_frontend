    ...
    const loadData = async () => {
      try {
        ...
        const [ohlcResult, signalsResult, priceResult] = await Promise.allSettled([
          fetchData(buildApiUrl(`/api/public/ohlc?symbol=${symbol}&interval=${timeframe}&limit=104`)),
          fetchData(buildApiUrl(`/api/public/signals/alerts?ticker=${symbol}&timeframe=1W`)),
          fetchData(buildApiUrl(`/api/public/market/price/${symbol}`))
        ]);

        if (!isMounted) return;

        const ohlcResponse = ohlcResult.status === 'fulfilled' ? ohlcResult.value : null;
        const signalsResponse = signalsResult.status === 'fulfilled' ? signalsResult.value : null;
