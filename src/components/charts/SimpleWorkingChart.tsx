...
export default function SimpleWorkingChart({ 
  ...
  const { data: marketData } = useQuery({
    ...
    queryFn: async () => {
      const apiUrl = import.meta.env.VITE_API_BASE_URL || 'https://crypto-kings-backend.vercel.app';
      const response = await fetch(`${apiUrl}/api/public/market/price/${symbol}`);
  ...
  const { data: ohlcData } = useQuery({
    queryKey: [`/api/public/ohlc`, symbol],
    queryFn: async () => {
      const apiUrl = import.meta.env.VITE_API_BASE_URL || 'https://crypto-kings-backend.vercel.app';
      const response = await fetch(`${apiUrl}/api/public/ohlc?symbol=${symbol}&interval=1w&limit=100`);
