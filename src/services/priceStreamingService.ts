...
class PriceStreamingService extends SimpleEventEmitter {
  ...
  private startTime = Date.now(); // Track connection session start time
  ...
  private setupErrorHandling() {
    this.on('error', (error: any) => {
      ...
      // Only attempt reconnection if enabled
  ...
  public connectBinanceWebSocket(symbols: string[] = ['btcusdt', 'ethusdt', 'solusdt']) {
    try {
      // Enable reconnection behavior for new connection attempts
      ...
      // Disconnect existing connection first with proper close code
      if (this.binanceWs) {
        this.binanceWs.close(1000, 'reconnect'); // Use standard close code to prevent spurious reconnects
        this.binanceWs = null;
      }
      ...
      this.binanceWs.onopen = () => {
        ...
        this.startTime = Date.now(); // Reset session start time on new connection
  ...
  private handleConnectionFailure() {
    // Bail out early if reconnection is disabled
    if (!this.shouldReconnect) {
      return;
    }
  ...
  public disconnect() {
    ...
    if (this.binanceWs) {
      this.binanceWs.close(1000, 'client disconnect'); // Use standard close code for intentional disconnection
      this.binanceWs = null;
    }
    ...
    // Clear cached throttle and price data
    this.updateThrottle = {};
